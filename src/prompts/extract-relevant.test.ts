...
describe("extractRelevent function", () => {
  ...
  it("does not throw when no code block exists", async () => {
    (extractCodeBlocks as jest.Mock).mockReturnValue([]);
    await expect(extractRelevent("plain text")).resolves.toBeUndefined();
  });

  it("does not throw when the selection is invalid", async () => {
    (extractCodeBlocks as jest.Mock).mockReturnValue(["first", "second"]);
    prompts.mockResolvedValueOnce({ wantsCodeBlock: "c" });
    prompts.mockResolvedValueOnce({ index: "99" });
    await expect(extractRelevent("text")).resolves.toBeUndefined();
  });
});
